...
The initial prototype version of a C# Mapiepire WebSocket Client class was created by Richard Schoen - MobiGoGo LLC and is named: ```MobiMapepireClient``` in file ```MobiMapepireClient.cs```. The class can be included in any current .net version project. It has only been tested with .Net 8.0 so far, not the .Net Framework. 
